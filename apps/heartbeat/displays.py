...
class MonthGridDisplay(ListDisplay):
    """Full-month calendar grid showing daily uptime by day."""
    ...
    template_name = "heartbeat/displays/month_grid.html"
    ...
    def get_context(self, queryset, crud_config, request):
        ...
        weeks = []
        current_week = [None] * first_weekday  # pad start
        ...
        for day_num in range(1, last_day_num + 1):
            date = today.replace(day=day_num)
            d = records.get(date)
            is_future = date > today
            ...
            if d:
                ...
                meets_sla = uptime >= minimum
                month_ok += d.ok_count
                month_fail += d.fail_count
                if d.avg_response_ms:
                    month_ms_total += d.avg_response_ms
                    month_ms_count += 1
                if meets_sla:
                    days_met += 1
                else:
        ...
        # Month aggregate
        month_total = month_ok + month_fail
        month_uptime = round(month_ok / month_total * 100, 2) if month_total else 0
        month_avg_ms = round(month_ms_total / month_ms_count) if month_ms_count else 0
        ...
        return {
            ...
            "month_total_fmt": _fmt_count(month_total),
            "month_avg_ms": month_avg_ms,
