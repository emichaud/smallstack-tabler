...
Every HTTP response includes an `X-Request-ID` header — a unique identifier for that specific request. This is generated by the `RequestIDMiddleware` and stored in the activity log's `RequestLog` table.

**Why this matters:** When a user reports "I got an error," you can ask for the request ID from their browser's network tab (or your frontend can log it automatically). Then search the activity log by that ID to find the exact request, its status code, response time, and which user triggered it.
